...
6. служба `systemd` запускает
    - целевое состояние по умолчанию - определяет, какие сервисы и процессы будут запущены при старте системы
    - службы
...
- `namespaces` - пространства имен - механизм обеспечивающий изоляцию процессов благодаря ограничению видимости тех или
  иных ресурсов ядра Linux путем предоставления процессам собственные экземпляры имен
    - в linux имеются 7 типов пространств имен:
        - cgroups
            - Изолирует и управляет ресурсами, такими как CPU, память и ввод-вывод, которые могут использовать процесс
        - pid
            - Изолирует идентификаторы процессов, позволяя каждому пространству имен иметь свой собственный набор PID
        - network
            - Изолирует сетевые интерфейсы, IP-адреса, маршруты и порты
        - user
            - Позволяет процессам иметь разные UID и GID в разных пространствах имен
        - mount
            - Изолирует точки монтирования файловых систем
        - ipc
            - Изолирует механизмы межпроцессного взаимодействия, такие как очереди сообщений, семафоры и разделяемая
              память
        - uts
            - Изолирует системное имя узла(hostname) и доменное имя
